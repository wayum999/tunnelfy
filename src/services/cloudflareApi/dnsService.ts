...
export class DnsService extends BaseCloudflareService {
   ...
   */
  async checkCnameConflicts(
    zoneId: string,
    recordName: string,
    tunnelId: string,
  ): Promise<CnameConflictResult> {
    try {
      ...
      const tunnelCname = records.find(
        (r) =>
          r.type === "CNAME" &&
      ...
      this.logger.error(
        LogComponent.API,
        "Failed to check CNAME conflicts:",
        error,
      );
      throw error;
    }
