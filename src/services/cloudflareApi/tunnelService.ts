...
export class TunnelService extends BaseCloudflareService {
    ...
    async getTunnelToken(tunnelId: string): Promise<string> {
        try {
            const accountId = await this.getAccountId();
            const response = await this.makeRequest<string>(`/accounts/${accountId}/cfd_tunnel/${tunnelId}/token`);
    ...
    async getTunnelConfigs(tunnelId: string): Promise<any> {
        try {
            const accountId = await this.getAccountId();
            const configs = await this.makeRequest<any>(`/accounts/${accountId}/tunnels/${tunnelId}/configurations`);
            return configs;
        } catch (error) {
