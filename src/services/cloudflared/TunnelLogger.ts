...
export class TunnelLogger {
  ...
  async logTunnelEvent(
    ...
    this.baseLogger.info(LogComponent.TUNNEL, logMessage.trim(), {
      preserveFocus,
    });
  ...
  async cleanupOldLogs(): Promise<void> {
    try {
      ...
      this.baseLogger.error(
        LogComponent.TUNNEL,
        `Error cleaning up old logs: ${error}`,
      );
    }
  }
