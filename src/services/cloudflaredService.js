...
exports.CloudflaredService = void 0;
...
class CloudflaredService {
    ...
    constructor(context) {
        ...
        this._onTunnelEvent = new vscode.EventEmitter();
        this.onTunnelEvent = this._onTunnelEvent.event;
        this.cloudflaredDir = path.join(os.homedir(), '.cloudflared');
        this.logger = logger_1.Logger.getInstance();
        this.tokenService = new tokenService_1.TokenService(context);
        this.logger.info(logger_1.LogComponent.TUNNEL, 'CloudflaredService initialized');
        this.logger.debug(logger_1.LogComponent.TUNNEL, `Using cloudflared directory: ${this.cloudflaredDir}`);
    }
    /**
     ...
     */
    async verifyCertFile() {
        const certPath = this.certPath;
        if (!fs.existsSync(certPath)) {
            this.logger.error(logger_1.LogComponent.TUNNEL, `Certificate file not found: ${certPath}`);
        ...
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, `Certificate file not accessible: ${error}`);
     ...
     */
    async runCloudflaredCommand(command) {
        try {
            ...
            this.logger.debug(logger_1.LogComponent.COMMAND, `Running command: ${command}`);
            const { stdout, stderr } = await util.promisify(cp.exec)(command, { env });
            if (stderr) {
                this.logger.warn(logger_1.LogComponent.COMMAND, 'Command stderr:', stderr.trim());
            }
            // For JSON responses, just log that we received data
            if (stdout.trim().startsWith('{') || stdout.trim().startsWith('[')) {
                this.logger.debug(logger_1.LogComponent.COMMAND, 'Command returned JSON data');
            }
            else {
                this.logger.debug(logger_1.LogComponent.COMMAND, 'Command output:', stdout.trim());
            }
            return { stdout, stderr };
        }
        catch (error) {
            const err = error;
            this.logger.error(logger_1.LogComponent.COMMAND, `Command failed: ${command}`, err);
            throw err;
        }
    }
    /**
    ...
    async checkInstallation() {
        try {
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Cloudflared installation check: ${isInstalled}`);
            return isInstalled;
        }
        catch (error) {
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to check cloudflared installation:', error);
            return false;
        }
    }
    /**
    ...
    async login() {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to login:', error);
            return false;
        }
    }
    /**
    ...
    async createTunnel(name) {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to create tunnel:', error);
            return false;
        }
    }
    /**
    ...
    async deleteTunnel(tunnelId) {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to delete tunnel:', error);
            return false;
        }
    }
    /**
    ...
    async listTunnels() {
        try {
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Listing tunnels...');
            const { stdout } = await this.runCloudflaredCommand('cloudflared tunnel list --output json');
            const tunnels = JSON.parse(stdout);
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Found ${tunnels.length} tunnels`);
            // Only log individual tunnels at debug level
            tunnels.forEach(tunnel => {
                this.logger.debug(logger_1.LogComponent.TUNNEL, `Found tunnel: ${tunnel.name} (${tunnel.id})`);
            });
            return tunnels;
        }
        catch (error) {
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to list tunnels:', error);
            return [];
        }
    }
    /**
     ...
     */
    async routeTunnel(tunnelId, hostname) {
        try {
            await this.runCloudflaredCommand(`cloudflared tunnel route dns ${tunnelId} ${hostname}`);
            return true;
        }
        catch (error) {
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to route tunnel:', error);
            return false;
        }
    }
    /**
     ...
     */
    async runTunnel(tunnelId, port, hostname) {
        this.logger.info(logger_1.LogComponent.TUNNEL, `Running tunnel ${tunnelId} on port ${port}`);
        try {
            // Get the token first
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Getting tunnel token...');
            const token = await this.getTunnelToken(tunnelId);
            if (!token) {
                const errorMessage = 'Failed to get tunnel token';
                this.logger.error(logger_1.LogComponent.TUNNEL, errorMessage);
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Got token for tunnel ${tunnelId}`);
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Resolved origin URL: ${hostname}`);
            ...
            this.logger.debug(logger_1.LogComponent.COMMAND, `Running cloudflared with args: ${args.join(' ')}`);
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Spawning cloudflared process...');
            ...
            tunnel.unref();
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Cloudflared process spawned and detached');
            ...
            const stdoutWatcher = fs.watch(stdoutLog, (eventType) => {
                if (eventType === 'change') {
                    ...
                    for (const line of lines) {
                        if (!line)
                            continue;
                        this.logger.debug(logger_1.LogComponent.TUNNEL, `Tunnel stdout: ${line}`);
                        ...
                        if (connMatch) {
                            ...
                            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Tunnel connected successfully, refreshing tunnel list...');
                            this.listTunnels().catch(err => {
                                this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to refresh tunnel list:', err);
                            });
                        }
            ...
            const stderrWatcher = fs.watch(stderrLog, (eventType) => {
                if (eventType === 'change') {
                    ...
                    for (const line of lines) {
                        if (!line)
                            continue;
                        this.logger.error(logger_1.LogComponent.TUNNEL, `Tunnel stderr: ${line}`);
                        this._onTunnelEvent.fire({
                            type: 'error',
                            tunnelId,
                            data: { error: line }
            ...
            tunnel.on('exit', (code, signal) => {
                ...
                this.logger.debug(logger_1.LogComponent.TUNNEL, message);
                stdoutWatcher.close();
                stderrWatcher.close();
                ...
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'Tunnel process exited, refreshing tunnel list...');
                this.listTunnels().catch(err => {
                    this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to refresh tunnel list:', err);
                });
            });
            ...
            tunnel.on('error', (error) => {
                ...
                this.logger.error(logger_1.LogComponent.TUNNEL, message);
                this._onTunnelEvent.fire({
                    type: 'error',
                    tunnelId,
                    data: { error: message }
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to run tunnel:', error);
            this._onTunnelEvent.fire({
                type: 'error',
                tunnelId,
    ...
    async getTunnelToken(tunnelId) {
        try {
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Getting token for tunnel ${tunnelId}`);
            try {
                // First try to get from secure storage
                const token = await this.tokenService.getTunnelToken(tunnelId);
                if (token) {
                    this.logger.debug(logger_1.LogComponent.TUNNEL, 'Found token in storage');
                    return token;
                }
            }
            catch (error) {
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'Failed to get token from storage:', error);
                // Continue to try getting from cloudflared
            }
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Getting token from cloudflared');
            // Get from cloudflared and store it
            const { stdout } = await this.runCloudflaredCommand(`cloudflared tunnel token ${tunnelId}`);
            const token = stdout.trim();
            if (token) {
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'Got token from cloudflared, storing it');
                try {
                    await this.tokenService.storeTunnelToken(tunnelId, token);
                }
                catch (storeError) {
                    this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to store token:', storeError);
                    // Continue even if we can't store it
                }
                return token;
            }
            else {
                this.logger.error(logger_1.LogComponent.TUNNEL, 'Got empty token from cloudflared');
                return null;
            }
        }
        catch (error) {
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to get tunnel token:', error);
            return null;
        }
    }
    /**
    ...
    async stopTunnel(tunnelId) {
        this.logger.info(logger_1.LogComponent.TUNNEL, `Stopping tunnel ${tunnelId}`);
        try {
            ...
            if (tunnelProcess) {
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'Found running tunnel process, terminating...');
                // Try SIGTERM first for graceful shutdown
                if (tunnelProcess.pid) {
                    this.logger.debug(logger_1.LogComponent.TUNNEL, `Sending SIGTERM to process group ${-tunnelProcess.pid}`);
                    process.kill(-tunnelProcess.pid, 'SIGTERM');
                }
                else {
                    this.logger.error(logger_1.LogComponent.TUNNEL, 'Tunnel process PID is undefined; cannot send SIGTERM');
                }
                ...
                await new Promise((resolve) => {
                    const timeout = setTimeout(() => {
                        ...
                        if (!tunnelProcess.killed && tunnelProcess.pid) {
                            this.logger.debug(logger_1.LogComponent.TUNNEL, `Process still running after SIGTERM, sending SIGKILL to ${-tunnelProcess.pid}`);
                            process.kill(-tunnelProcess.pid, 'SIGKILL');
                        }
                        resolve();
                    }, 5000); // 5 second timeout
                    tunnelProcess.once('exit', () => {
                        this.logger.debug(logger_1.LogComponent.TUNNEL, 'Process exited successfully');
                ...
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'No running process found for tunnel');
            }
        }
        catch (err) {
            const error = err instanceof Error ? err : new Error('Unknown error occurred');
            this.logger.error(logger_1.LogComponent.TUNNEL, `Failed to stop tunnel ${tunnelId}:`, error);
            this._onTunnelEvent.fire({
                type: 'error',
                tunnelId,
                data: { error: error.message }
    ...
    async getTunnelInfo(tunnelId) {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to get tunnel info:', error);
            return null;
        }
    }
    /**
    ...
    async checkTunnelStatus(tunnelId) {
        try {
            ...
            try {
                ...
                this.logger.error(logger_1.LogComponent.TUNNEL, 'Error parsing tunnel list:', parseError);
            }
            ...
            try {
                ...
                this.logger.error(logger_1.LogComponent.TUNNEL, `Could not get tunnel info: ${connError}`);
            }
            return false;
        }
        catch (error) {
            this.logger.error(logger_1.LogComponent.TUNNEL, `Error checking tunnel status: ${error}`);
            return false;
        }
    }
    /**
    ...
    async cleanupTunnels() {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to cleanup tunnels:', error);
        }
    }
    /**
     ...
     */
    async createQuickTunnel(port, hostname) {
        try {
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Starting quick tunnel creation for port ${port}`);
            // Verify cert file first
            await this.verifyCertFile();
            this.logger.debug(logger_1.LogComponent.TUNNEL, 'Certificate verified');
            // Run tunnel in the background
            const args = ['tunnel', '--url', `http://localhost:${port}`];
            if (hostname) {
                args.push('--hostname', hostname);
            }
            this.logger.debug(logger_1.LogComponent.COMMAND, `Spawning cloudflared with args: ${args.join(' ')}`);
            const tunnel = cp.spawn('cloudflared', args, {
                stdio: 'pipe',
                env: {
                    ...process.env,
            ...
            return new Promise((resolve, reject) => {
                ...
                let timeoutId;
                this.logger.debug(logger_1.LogComponent.TUNNEL, 'Setting up tunnel output handlers');
                tunnel.stdout?.on('data', (data) => {
                    const line = data.toString();
                    output += line;
                    this.logger.debug(logger_1.LogComponent.TUNNEL, `Quick tunnel stdout: ${line}`);
                    // Look for the tunnel URL in the output
                    const tunnelMatch = line.match(/https:\/\/[^\s]+\.trycloudflare\.com/);
                    if (tunnelMatch && !tunnelUrl) {
                        tunnelUrl = tunnelMatch[0];
                        this.logger.debug(logger_1.LogComponent.TUNNEL, `Found tunnel URL: ${tunnelUrl}`);
                    }
                    if (tunnelUrl) {
                        this.logger.debug(logger_1.LogComponent.TUNNEL, 'Found tunnel URL, resolving promise');
                ...
                tunnel.stderr?.on('data', (data) => {
                    const line = data.toString();
                    this.logger.debug(logger_1.LogComponent.TUNNEL, `Quick tunnel stderr: ${line}`);
                    // Also look for tunnel URL in stderr
                    const tunnelMatch = line.match(/https:\/\/[^\s]+\.trycloudflare\.com/);
                    if (tunnelMatch && !tunnelUrl) {
                        tunnelUrl = tunnelMatch[0];
                        this.logger.debug(logger_1.LogComponent.TUNNEL, `Found tunnel URL in stderr: ${tunnelUrl}`);
                        clearTimeout(timeoutId);
                        resolve({
                            url: localUrl,
                            tunnelUrl: tunnelUrl
                ...
                tunnel.on('error', (error) => {
                    this.logger.error(logger_1.LogComponent.TUNNEL, 'Quick tunnel error:', error);
                    clearTimeout(timeoutId);
                    reject(error);
                });
                tunnel.on('exit', (code) => {
                    this.logger.debug(logger_1.LogComponent.TUNNEL, `Quick tunnel process exited with code ${code}`);
                    if (code !== 0 && !tunnelUrl) {
                        clearTimeout(timeoutId);
                ...
                timeoutId = setTimeout(() => {
                    this.logger.error(logger_1.LogComponent.TUNNEL, 'Timeout waiting for quick tunnel URL');
                    tunnel.kill();
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to create quick tunnel:', error);
            return null;
        }
    }
    /**
    ...
    async stopQuickTunnel(port) {
        try {
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to stop quick tunnel:', error);
            return false;
        }
    }
}
exports.CloudflaredService = CloudflaredService;
//# sourceMappingURL=cloudflaredService.js.map