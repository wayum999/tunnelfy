...
const tokenAuditService_1 = require("./tokenAuditService");
class TokenService {
    /**
    ...
    constructor(context) {
        ...
        this.auditService = new tokenAuditService_1.TokenAuditService(context);
    }
    /**
     ...
     */
    decryptFromMemory(encrypted, iv, authTag) {
     ...
     */
    async storeTunnelToken(tunnelId, token) {
        try {
            await this.checkFailedAttempts();
            // Store token in VS Code's secure storage
            await this.context.secrets.store(TokenService.TUNNEL_TOKEN_PREFIX + tunnelId, token);
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Stored token for tunnel ${tunnelId}`);
        }
        catch (error) {
            await this.auditService.recordEvent({
                action: 'create',
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to store tunnel token:', error);
    ...
    async getTunnelToken(tunnelId) {
        try {
            ...
            if (memoryToken) {
                const token = this.decryptFromMemory(memoryToken.encrypted, memoryToken.iv, memoryToken.authTag);
                await this.auditService.recordEvent({
                    action: 'access',
                    tunnelId,
                    success: true
            ...
            const token = await this.context.secrets.get(TokenService.TUNNEL_TOKEN_PREFIX + tunnelId);
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to retrieve tunnel token:', error);
     ...
     */
    async deleteTunnelToken(tunnelId) {
        try {
            await this.checkFailedAttempts();
            // Remove token from VS Code's secure storage
            await this.context.secrets.delete(TokenService.TUNNEL_TOKEN_PREFIX + tunnelId);
            ...
            this.logger.debug(logger_1.LogComponent.TUNNEL, `Deleted token for tunnel ${tunnelId}`);
        }
        catch (error) {
            await this.auditService.recordEvent({
                action: 'delete',
            ...
            this.logger.error(logger_1.LogComponent.TUNNEL, 'Failed to delete tunnel token:', error);
    ...
    async copyTokenToClipboard(token) {
        try {
            ...
            const response = await vscode.window.showWarningMessage('The tunnel token will be copied to your clipboard and automatically cleared after 30 seconds. ' +
                'Make sure to use it before then.', 'Continue', 'Cancel');
            ...
            const timeout = setTimeout(async () => {
                ...
                if (currentClipboard === token) {
                    ...
                    this.logger.debug(logger_1.LogComponent.TUNNEL, 'Cleared token from clipboard');
                }
