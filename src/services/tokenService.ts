...
export class TokenService {
  ...
  private encryptedTokens = new Map<
    string,
   ...
   */
  private encryptForMemory(data: string): {
    encrypted: Buffer;
    iv: Buffer;
    authTag: Buffer;
  } {
    ...
    const cipher = crypto.createCipheriv(
      TokenService.ENCRYPTION_ALGORITHM,
      this.memoryKey,
      iv,
    );
  ...
  private async checkFailedAttempts(): Promise<void> {
    ...
    if (
      now - this.lastFailedAttempt >
      TokenService.FAILED_ATTEMPTS_TIMEOUT_MS
    ) {
  ...
  async storeTunnelToken(tunnelId: string, token: string): Promise<void> {
    try {
      ...
      this.logger.debug(
        LogComponent.TUNNEL,
        `Stored token for tunnel ${tunnelId}`,
      );
    } catch (error: any) {
      await this.auditService.recordEvent({
        action: "create",
  ...
  async getTunnelToken(tunnelId: string): Promise<string | undefined> {
    try {
      ...
      this.logger.error(
        LogComponent.TUNNEL,
        "Failed to retrieve tunnel token:",
        error,
      );
  ...
  async deleteTunnelToken(tunnelId: string): Promise<void> {
    try {
      ...
      this.logger.debug(
        LogComponent.TUNNEL,
        `Deleted token for tunnel ${tunnelId}`,
      );
    } catch (error: any) {
      await this.auditService.recordEvent({
        action: "delete",
