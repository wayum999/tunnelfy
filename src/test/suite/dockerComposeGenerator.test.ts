...
suite("DockerComposeGenerator Test Suite", () => {
  ...
  setup(async () => {
    ...
    dockerComposeGenerator = new DockerComposeGenerator(
      mockTunnelManager,
      mockApiService,
    );
  });
  ...
  test("should generate Docker Compose file with correct content", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        content.includes("image: cloudflare/cloudflared:latest"),
        true,
      );
      assert.strictEqual(
        content.includes(`- cloudflare.${testTunnelName}.env`),
        true,
      );
  ...
  test("should generate environment file with correct content", async () => {
    ...
    try {
      const composePath = await dockerComposeGenerator.generateComposeFile(
        testTunnelId,
        testTunnelName,
        testPort,
      );
  ...
  test("should handle network configuration documentation", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        content.includes("http://host.docker.internal:"),
        true,
      );
    } finally {
      vscode.window.showInformationMessage = originalShowInfo;
  ...
  test("should handle errors gracefully", async () => {
    ...
    try {
      ...
      await assert.rejects(
        () =>
          dockerComposeGenerator.generateComposeFile(
            testTunnelId,
            testTunnelName,
            testPort,
          ),
  ...
  test("should not generate files when confirmation is cancelled", async () => {
    ...
    try {
      const result = await dockerComposeGenerator.generateComposeFile(
        testTunnelId,
        testTunnelName,
        testPort,
      );
  ...
  test("should show overwrite confirmation when files exist", async () => {
    ...
    try {
      // Create files first
      await dockerComposeGenerator.generateComposeFile(
        testTunnelId,
        testTunnelName,
        testPort,
      );
