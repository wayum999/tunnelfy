...
suite('TunnelLogger Test Suite', () => {
    ...
    test('should create log stream for tunnel', async () => {
        ...
        assert.strictEqual(content, testMessage, 'Log file should contain written message');
    });
    ...
    test('should log tunnel events', async () => {
        ...
        assert.strictEqual(loggedMessages[0].component, LogComponent.TUNNEL, 'Should be tunnel component');
        assert.ok(loggedMessages[0].message.includes(tunnelId), 'Should include tunnel ID');
        assert.ok(loggedMessages[0].message.includes(event), 'Should include event');
        assert.ok(loggedMessages[0].message.includes(JSON.stringify(details)), 'Should include details');

        // Check file contents
        const logFile = path.join(logDir, `${tunnelId}.log`);
        const content = await fs.promises.readFile(logFile, 'utf8');
        assert.ok(content.includes(event), 'Log file should contain event');
        assert.ok(content.includes(JSON.stringify(details)), 'Log file should contain details');
    });
    ...
    test('should rotate logs when file size exceeds limit', async function() {
        ...
        try {
            ...
            while (attempt < maxAttempts && !rotated) {
                ...
                await testLogger.logTunnelEvent(tunnelId, 'test-event', { data: chunk.toString() });
                ...
                if (logFiles.length >= 2) {
                    ...
                    // Verify rotation
                    assert.ok(logFiles.includes(`${tunnelId}.log`), 'Should have current log file');
                    assert.ok(logFiles.includes(`${tunnelId}.1.log`), 'Should have rotated log file');
                    
                    // Verify file contents
                    const currentLog = await fs.promises.readFile(path.join(logDir, `${tunnelId}.log`), 'utf8');
                    const rotatedLog = await fs.promises.readFile(path.join(logDir, `${tunnelId}.1.log`), 'utf8');
    ...
    test('should clean up old logs', async () => {
        ...
        assert.strictEqual(fs.existsSync(logFile), false, 'Old log file should be deleted');
        assert.ok(
    ...
    test('should handle concurrent log operations', async () => {
        ...
        events.forEach(({ event, details }) => {
            ...
            assert.ok(content.includes(JSON.stringify(details)), `Log should contain details for ${event}`);
        });
    });
}); 