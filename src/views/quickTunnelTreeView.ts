 ...
 */
export class QuickTunnelTreeDataProvider
  implements vscode.TreeDataProvider<QuickTunnelTreeItem>
{
  private _onDidChangeTreeData: vscode.EventEmitter<
    QuickTunnelTreeItem | undefined | null | void
  > = new vscode.EventEmitter<QuickTunnelTreeItem | undefined | null | void>();
  ...
  private setupAutoRefresh(): void {
    ...
    if (!autoRefreshEnabled) {
      this.logger.debug(
        LogComponent.TUNNEL,
        "Auto-refresh disabled by configuration",
      );
      return;
    }
    ...
    const intervalSeconds = Math.max(
      5,
      Math.min(300, config.get<number>("autoRefreshInterval", 30)),
    );
    ...
    this.logger.debug(
      LogComponent.TUNNEL,
      `Auto-refresh set up with interval: ${intervalSeconds}s`,
    );
  }
  ...
  async getChildren(): Promise<QuickTunnelTreeItem[]> {
    try {
      ...
      this.logger.error(
        LogComponent.EXTENSION,
        `Failed to get quick tunnels: ${error}`,
      );
      return [];
    }
  }
  ...
  async addQuickTunnel(portOrUrl: number | string, name?: string): Promise<void> {
    try {
      ...
      this.logger.error(
        LogComponent.EXTENSION,
        `Failed to add quick tunnel: ${error}`,
      );
      throw error;
    }
  }
  ...
  async removeQuickTunnel(port: number): Promise<void> {
    try {
      ...
      this.logger.error(
        LogComponent.EXTENSION,
        `Failed to remove quick tunnel: ${error}`,
      );
      throw error;
    }
  }
